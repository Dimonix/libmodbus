...
int modbus_get_tid(modbus_t *ctx, uint8_t *req, int req_length){
    return ctx->backend->prepare_response_tid(req, &req_length);
}

int modbus_reply_raw_response(modbus_t *ctx, int tid, uint8_t *raw_rsp, int raw_rsp_length) {
    sft_t sft;
    ...
    if (raw_rsp_length < 2 || raw_rsp_length > (MODBUS_MAX_PDU_LENGTH + 1)) {
        /* The raw response must contain function and slave at least and
           must not be longer than the maximum pdu length plus the slave
           address. */
        errno = EINVAL;
        return -1;
